...
@interface MiddleAddViewController ()  <AVAudioPlayerDelegate>
...
@implementation MiddleAddViewController


//音量加
- (IBAction)VoideAdd:(id)sender {

    if (_player.volume + 0.1 <= 3.0) {
...
//音量减
- (IBAction)VoideReduce:(id)sender {
    
    if (_player.volume + 0.1 > 0.001) {
...
- (IBAction)previous:(id)sender {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SendReduceIndex" object:nil];
...
- (IBAction)next:(id)sender {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SendAddIndex" object:nil];
